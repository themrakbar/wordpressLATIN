...
function load(result) {
  ...
  (0,_store_actions_load_components_assets__WEBPACK_IMPORTED_MODULE_1__.loadComponentsAssets)(containers.map(container => container.model.toJSON()));
}
...
async function getComponentIds(elements, cache, isRecursive) {
  const results = await Promise.all(elements.map(async ({
    widgetType,
    elType,
